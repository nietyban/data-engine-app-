        ...
        {['LEAD','DC','DA'].map(role => (
          <div key={role} style={{ marginBottom:'14px' }}>
            ...
            <div style={{ display:'flex', flexWrap:'wrap', gap:'6px' }}>
              {shiftRoster.filter(m => m.role === role).map(m => {
                const out = absentIds.has(m.id)
                return (
                  <div key={m.id}
                    onClick={() => toggle(m.id, user.id)}
                    style={{ padding:'6px 12px', borderRadius:'99px',
                      border: `1px solid ${out ? '#fca5a5' : '#d1fae5'}`,
                      background: out ? '#fef2f2' : '#f0fdf4',
                      color: out ? '#dc2626' : '#16a34a',
